...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const allowed = ["processed_flag", "cleaned_text", "source_type", "language", "interview_id", "father_profile_id"];
  ...
  if ("processed_flag" in updates) {
    await insertAuditLog({
      user_id: user.id,
      action: updates.processed_flag ? "transcript.mark_reviewed" : "transcript.mark_pending",
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const admin = createAdminClient();
  const { error } = await admin.from("transcripts").delete().eq("id", id);
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
